...
class NotesWriter:
    ...
    def _format_markdown(self, data, findings):
        ...
        # HEADER
        # ============================================================
        md.append("# GRUDARIN - Network Monitor (Spy) Session Report")
        md.append("")
        md.append(f"> Generated by Grudarin v{__version__} - The powerful network spy tool.")
        ...
        # ============================================================
        # VISITOR ACTIVITY (Site Scan specific)
        # ============================================================
        visitors = [v for v in data.get("devices", {}).values() if v.get("node_type") == "VISITOR"]
        if visitors:
            md.append("## Local Visitors to Target Site")
            md.append("")
            md.append("| Visitor IP | Activity | First Seen |")
            md.append("|------------|----------|------------|")
            for v in visitors:
                md.append(f"| `{v['ip']}` | Visited Target Domain | `{v['first_seen']}` |")
            md.append("")
